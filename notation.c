...
int notation(s21_SmartCalc src, s21_SmartCalc data){
    ...
    for(int i = 0; src[i].num != -1; i++){
        ...
        if((src[i].type == 20 && src[i - 1].type == 18)){
        ...
        if(src[i].priority == 0){          // если число, сразу кладем в выходную строку
            ...
            count_stack++;
        } else if(peek_priority(src + i) > peek_priority(stack + count_stack - 1) && src[i].type != 19){
...
void push(s21_SmartCalc src, double value) {
    src->value = value;
    src->num++;
  }
